...
float cameraDistance = 5.0f; // Odległość kamery od punktu (0,0,0)

float yaw = -90.0f; // obrót na boki
float pitch = 0.0f;  // obrót w górę/dół
float lastX, lastY;
bool mouseDown = false;

// Zmienne do obsługi dotyku (zoomu)
...
// Zmienne do rysowania 2D (d-pad)
GLuint uiProgram = 0;
GLuint uiVBO, uiIBO;
GLint uiUniformColorLoc;
GLint uiUniformMVPLoc;
...
// --- Funkcje pomocnicze do ładowania zasobów (bez zmian) ---
...
// Funkcja do rysowania kwadratu/przycisku d-pada
void drawSquare(float x, float y, float size, glm::vec4 color) {
    ...
    glUniform4fv(uiUniformColorLoc, 1, glm::value_ptr(color));
    
    GLint posLoc = glGetAttribLocation(uiProgram, "aPos");
    glEnableVertexAttribArray(posLoc);
    glVertexAttribPointer(posLoc, 2, GL_FLOAT, GL_FALSE, sizeof(float) * 2, (void*)0);
...
bool init() {
    ...
    // Inicjalizacja shadera dla UI
    GLuint uiVsId = compileShader(GL_VERTEX_SHADER, uiVs);
    GLuint uiFsId = compileShader(GL_FRAGMENT_SHADER, uiFs);
    uiProgram = glCreateProgram();
    glAttachShader(uiProgram, uiVsId);
    glAttachShader(uiProgram, uiFsId);
    glLinkProgram(uiProgram);

    uiUniformColorLoc = glGetUniformLocation(uiProgram, "uColor");
    uiUniformMVPLoc = glGetUniformLocation(uiProgram, "MVP");
...
void render() {
    ...
    // 1. Macierz Projekcji (stała)
    glm::mat4 projection = glm::perspective(glm::radians(45.0f), 640.0f / 480.0f, 0.1f, 100.0f);
    
    // 2. Macierz Widoku (kamery orbitalnej)
    glm::vec3 front;
    front.x = cos(glm::radians(yaw)) * cos(glm::radians(pitch));
    front.y = sin(glm::radians(pitch));
    front.z = sin(glm::radians(yaw)) * cos(glm::radians(pitch));
    cameraFront = glm::normalize(front);
    cameraPos = cameraDistance * (-cameraFront);
    glm::mat4 view = glm::lookAt(cameraPos, glm::vec3(0.0f, 0.0f, 0.0f), cameraUp);

    // 3. Macierz Modelu (statyczna pozycja)
    ...
    // --- Renderowanie interfejsu 2D (d-pad) ---
    glDisable(GL_DEPTH_TEST);
    glUseProgram(uiProgram);
    
    glm::mat4 ui_projection = glm::ortho(0.0f, screenWidth, screenHeight, 0.0f, -1.0f, 1.0f);
    glUniformMatrix4fv(uiUniformMVPLoc, 1, GL_FALSE, glm::value_ptr(ui_projection));
    ...
    // Kółko
    drawSquare(dpadX, dpadY, dpadSize * 3, glm::vec4(0.5f, 0.5f, 0.5f, 0.5f));
    
    // Przyciski d-pada
    drawSquare(dpadX, dpadY - dpadSize, dpadSize, glm::vec4(0.8f, 0.8f, 0.8f, 0.8f)); // Góra
    drawSquare(dpadX, dpadY + dpadSize, dpadSize, glm::vec4(0.8f, 0.8f, 0.8f, 0.8f)); // Dół
    drawSquare(dpadX - dpadSize, dpadY, dpadSize, glm::vec4(0.8f, 0.8f, 0.8f, 0.8f)); // Lewo
    drawSquare(dpadX + dpadSize, dpadY, dpadSize, glm::vec4(0.8f, 0.8f, 0.8f, 0.8f)); // Prawo
...
void main_loop2() {
    ...
    while (SDL_PollEvent(&e)) {
        ...
        // --- OBSŁUGA DOTYKU (obrót jednym palcem, zoom dwoma) ---
        else if (e.type == SDL_FINGERDOWN) {
            int numFingers = SDL_GetNumTouchFingers(e.tfinger.touchId);
            ...
            if (numFingers == 1) { 
                ...
                // Sprawdzamy czy dotyk nie jest na d-padzie
                if (touchX < dpadX - dpadSize * 1.5f || touchX > dpadX + dpadSize * 1.5f ||
                    touchY < dpadY - dpadSize * 1.5f || touchY > dpadY + dpadSize * 1.5f) {
                    mouseDown = true;
                    lastX = e.tfinger.x * screenWidth;
