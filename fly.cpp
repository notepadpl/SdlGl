...
void PrintAnimationsAndBones(const aiScene* scene) {
    ...
    for (unsigned int m = 0; m < scene->mNumMeshes; m++) {
        for (unsigned int b = 0; b < scene->mMeshes[m]->mBones; b++) {
...
void render() {
    ...
    // Obliczamy wektor kierunku kamery na podstawie yaw i pitch
    glm::vec3 front;
    ...
    // Ustawiamy macierz widoku, aby kamera patrzyła w kierunku `cameraFront`
        ...
        // --- OBSŁUGA DOTYKU ---
            ...
            if (numFingers == 1 && mouseDown) {
                ...
                if (finger1 && finger2 && initialFingerDistance > 0.001f) {
                    ...
                    // Nowa logika zoomu: przesuwamy kamerę wzdłuż jej wektora 'front'
                    cameraPos += cameraFront * (zoomDelta * zoomSpeed);
