...
float cameraRotX = 0.0f; // Kąt obrotu wokół osi X (góra/dół)
float cameraRotY = glm::radians(90.0f); // Kąt obrotu wokół osi Y (lewo/prawo), startowo 90 stopni, żeby patrzeć na plecy
float cameraDistance = 5.0f; // Dystans od modelu

// Punkt, wokół którego kamera orbituje. Ustawiony na 0.5, żeby kamera patrzyła na tors
...
void render() {
    ...
    // 1. Obliczanie macierzy widoku i projekcji
    glm::mat4 projection = glm::perspective(glm::radians(45.0f), 640.0f / 480.0f, 0.1f, 100.0f);
    
    // Obliczanie pozycji kamery na podstawie kątów i odległości
    glm::vec3 cameraPos(
        cameraTarget.x + cameraDistance * cos(cameraRotX) * sin(cameraRotY),
        cameraTarget.y + cameraDistance * sin(cameraRotX),
        cameraTarget.z + cameraDistance * cos(cameraRotX) * cos(cameraRotY)
    );

    // Ustawiamy macierz widoku, aby kamera patrzyła na `cameraTarget`
    glm::mat4 view = glm::lookAt(cameraPos, cameraTarget, cameraUp);
    
    // 2. Macierz modelu (statyczna pozycja)
...
void main_loop() {
    ...
    while (SDL_PollEvent(&e)) {
        ...
        // --- Sterowanie myszą (bez zmian) ---
            ...
            // Ruch w osi X myszy odpowiada obrotowi w osi Y (piruet)
            cameraRotY += (e.motion.x - lastX) * rotationSpeed;
            
            // Ruch w osi Y myszy odpowiada obrotowi w osi X (góra/dół)
            cameraRotX += (e.motion.y - lastY) * rotationSpeed; 

            // Ograniczanie obrotu w osi X
            cameraRotX = glm::clamp(cameraRotX, glm::radians(-89.0f), glm::radians(89.0f));
        ...
        // --- Sterowanie dotykiem (zmodyfikowana część) ---
            ...
            if (numFingers == 1 && mouseDown) {
                // Obliczanie różnicy pozycji palca
                float xOffset = e.tfinger.x * 640 - lastX;
                float yOffset = e.tfinger.y * 480 - lastY;
                
                // Rotacja horyzontalna (piruet) - ruch palca w osi X
                cameraRotY += xOffset * rotationSpeed;
                
                // Rotacja wertykalna (góra/dół) - ruch palca w osi Y
                cameraRotX += yOffset * rotationSpeed;
                
                // Ograniczenie obrotu w osi X
                cameraRotX = glm::clamp(cameraRotX, glm::radians(-89.0f), glm::radians(89.0f));
...
// ... (reszta kodu main(), init(), cleanup() itd. bez zmian)
void main_loop2() {
    SDL_Event e;
    while (SDL_PollEvent(&e)) {
        if (e.type == SDL_QUIT) {
        ...
        // --- Sterowanie myszą ---
            ...
            // Ruch w osi X myszy odpowiada obrotowi w osi Y
            cameraRotY += (e.motion.x - lastX) * rotationSpeed;
            // Ruch w osi Y myszy odpowiada obrotowi w osi X
            cameraRotX += (e.motion.y - lastY) * rotationSpeed;

            // Ograniczanie obrotu w osi X, żeby nie "wskoczyć" pod model
            cameraRotX = glm::clamp(cameraRotX, -glm::radians(89.0f), glm::radians(89.0f));
            ...
            cameraDistance = glm::clamp(cameraDistance, 1.0f, 15.0f); // Ograniczenie odległości
        }
        ...
        // --- Sterowanie dotykiem ---
            ...
            int numFingers = SDL_GetNumTouchFingers(e.tfinger.touchId);
            if (numFingers == 1 && mouseDown) {
                cameraRotY += (e.tfinger.x * 640 - lastX) * rotationSpeed;
                cameraRotX += (e.tfinger.y * 480 - lastY) * rotationSpeed;
                cameraRotX = glm::clamp(cameraRotX, -glm::radians(89.0f), glm::radians(89.0f));
                lastX = e.tfinger.x * 640;
                lastY = e.tfinger.y * 480;
            } else if (numFingers == 2) {
