...
// Zmienne dla kamery
// Nowa pozycja startowa na wprost
glm::vec3 cameraPos = glm::vec3(0.0f, 0.5f, 5.0f); // Podniesiona lekko, by widok nie był od stóp
glm::vec3 cameraTarget = glm::vec3(0.0f, 0.0f, 0.0f); // Cel kamery - środek modelu
...
void render() {
    ...
    // 2. Macierz Widoku (kamery)
    // Obliczamy wektor kierunku kamery
...
void main_loop() {
    ...
    while (SDL_PollEvent(&e)) {
        ...
        // --- OBSŁUGA MYSZY ---
            ...
            // Obliczamy wektory "prawo" i "góra" na podstawie aktualnego kierunku kamery
            glm::vec3 cameraDirection = glm::normalize(cameraTarget - cameraPos);
            glm::vec3 cameraRight = glm::normalize(glm::cross(cameraDirection, cameraUp));
            glm::vec3 newCameraUp = glm::cross(cameraRight, cameraDirection);

            // Rotacja kamery
            glm::quat rotX = glm::angleAxis(glm::radians(-yoffset), cameraRight);
            glm::quat rotY = glm::angleAxis(glm::radians(-xoffset), newCameraUp);
            glm::quat orientation = rotY * rotX;

            cameraPos = cameraTarget + orientation * (cameraPos - cameraTarget);
        }
        else if (e.type == SDL_MOUSEWHEEL) {
            if (e.wheel.y > 0) { // Przybliż
                cameraPos += glm::normalize(cameraTarget - cameraPos) * zoomSpeed;
        ...
        // --- OBSŁUGA DOTYKU ---
            ...
            if (numFingers == 1 && mouseDown) {
                ...
                float yoffset = lastY - e.tfinger.y * 480;
                ...
                if (finger1 && finger2 && initialFingerDistance > 0.001f) {
                    ...
                    cameraPos += glm::normalize(cameraTarget - cameraPos) * (zoomDelta * zoomSpeed);
