...
// Zmienne dla kamery (teraz prostsze, kamera jest bardziej statyczna)
float cameraPitch = 0.0f; // Kąt góra/dół (wokół osi X)
float cameraDistance = 5.0f; // Dystans od modelu
glm::vec3 cameraTarget = glm::vec3(0.0f, 0.5f, 0.0f); // Punkt, na który patrzy kamera
glm::vec3 worldUp = glm::vec3(0.0f, 1.0f, 0.0f); // Globalna oś "góry"

// NOWA ZMIENNA: Kąt obrotu MODELU wokół osi Y (piruet)
...
float rotationSpeed = 0.005f; // Czułość obrotu
...
uniform mat4 MVP;
uniform mat4 Model; // Teraz Model będzie zawierał rotację modelu
...
void PrintAnimationsAndBones(const aiScene* scene) {
    ...
    for (unsigned int m = 0; m < scene->mNumMeshes; m++) {
        if (scene->mMeshes[m]->mNumBones > 0) {
            for (unsigned int b = 0; b < scene->mMeshes[m]->mBones[b]->mName.C_Str(); b++) {
...
void render() {
    ...
    // 1. Obliczanie macierzy widoku (kamera jest uproszczona)
    glm::mat4 projection = glm::perspective(glm::radians(45.0f), 640.0f / 480.0f, 0.1f, 100.0f);
    
    // Obliczanie pozycji kamery
    // Kamera zawsze patrzy na cameraTarget, z góry/dołu, z określonej odległości
    glm::vec3 cameraPos(0.0f, 0.0f, cameraDistance); 
    // Obrót kamery w górę/dół (Pitch)
    glm::quat pitchRotation = glm::angleAxis(cameraPitch, glm::vec3(1.0f, 0.0f, 0.0f));
    cameraPos = pitchRotation * cameraPos;
    
    // Finalna pozycja kamery jest przesunięta o cameraTarget
    cameraPos += cameraTarget; 

    // Kamera patrzy na cameraTarget, a jej "góra" jest zawsze globalną osią Y
    glm::mat4 view = glm::lookAt(cameraPos, cameraTarget, worldUp);
    
    // 2. Macierz modelu (teraz to MODEL będzie się obracał)
    glm::mat4 model = glm::mat4(1.0f);
    // Skalowanie modelu
    model = glm::scale(model, glm::vec3(0.1f, 0.1f, 0.1f));
    
    // **KLUCZOWA ZMIANA:** Rotacja modelu wokół osi Y (piruet)
    model = glm::rotate(model, modelRotationY, glm::vec3(0.0f, 1.0f, 0.0f)); 
    
    // Przesunięcie modelu tak, aby jego "środek obrotu" był na cameraTarget (lub wyżej)
    // To jest ważne, jeśli model ma pivot w 0,0,0, ale chcesz, żeby obracał się wokół pasa np.
    // Jeśli cameraTarget jest (0, 0.5, 0), to przesuwamy model tak, aby 0.5 było jego środkiem obrotu.
    // W zasadzie to cameraTarget już to załatwia, ale możesz dostosować tu model,
    // jeśli jego lokalny pivot (0,0,0) nie odpowiada miejscu, wokół którego chcesz piruet.
    // Na razie zostawiamy, żeby obracał się wokół swojego lokalnego 0,0,0, które jest na ziemi.
    // Jeśli model ma "buty" w (0,0,0), a chcesz obracać wokół "pasa", to tu musisz go podnieść:
    // model = glm::translate(model, glm::vec3(0.0f, some_height_offset, 0.0f)); 
    // A następnie z powrotem sprowadzić w dół po wszystkich transformacjach,
    // albo po prostu ustawić cameraTarget na środek modelu, a model zostawić w 0,0,0.

    // 3. Połączenie macierzy
...
void main_loop() {
    ...
    while (SDL_PollEvent(&e)) {
        ...
        // --- Sterowanie myszą ---
            ...
            // KLUCZOWA ZMIANA: Ruch w osi X myszy (lewo/prawo) kontroluje OBRÓT MODELU (piruet)
            modelRotationY += (e.motion.x - lastX) * rotationSpeed;
            
            // Ruch w osi Y myszy (góra/dół) kontroluje KĄT KAMERY (pitch)
            cameraPitch -= (e.motion.y - lastY) * rotationSpeed; // Ujemny, aby ruch myszy w dół = kamera w dół
            
            // Ograniczanie kąta kamery, żeby nie przechodzić "za plecy"
        ...
        // --- Sterowanie dotykiem ---
            ...
            if (numFingers == 1 && mouseDown) {
                ...
                // KLUCZOWA ZMIANA: Piruet (obrót MODELU wokół globalnej osi Y)
                modelRotationY += xOffset * rotationSpeed;
                // Góra/dół (obrót KAMERY)
