...
// Nowe zmienne dla kamery
float cameraRotX = 0, cameraRotY = 0;
float cameraDistance = 3.0f;
bool mouseDown = false;
int lastX, lastY;

// Nowe lokalizacje uniformów
GLint uniformMVPLoc;
...
void Material::bind(GLuint program) const {
    ...
    // Na razie nie używamy tych map, ale warto mieć ten kod
...
void render() {
    ...
    // Obliczanie pozycji kamery na podstawie obrotów
    glm::vec3 cameraPos(
        cameraDistance * sin(cameraRotY) * cos(cameraRotX),
        cameraDistance * sin(cameraRotX),
        cameraDistance * cos(cameraRotY) * cos(cameraRotX)
    );
    glm::mat4 view = glm::lookAt(cameraPos, glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));

    // Macierz modelu - tutaj ustawiamy skalę obiektu
    glm::mat4 model = glm::mat4(1.0f);
    model = glm::scale(model, glm::vec3(0.5f, 0.5f, 0.5f)); // Skalujemy obiekt, aby nie był zbyt duży
...
void main_loop() {
    ...
    while (SDL_PollEvent(&e)) {
        if (e.type == SDL_QUIT) {
            ...
            // Ograniczenie obrotu, aby uniknąć "przewrotki"
