...
int lastX, lastY;
GLint uniformScaleLoc; // <--- Nowa zmienna dla lokalizacji skali
...
uniform float scale; // <-- Nowa zmienna
uniform float rotX, rotY;
...
void main(){
    ...
    vec3 p = Ry * Rx * aPos;
    gl_Position = vec4(p * scale, 1.0); // <--- Użycie zmiennej 'scale'
...
void Model::render(GLuint program, float rotX, float rotY) {
    ...
    glUniform1f(uniformScaleLoc, 0.3f); // <--- Dodano tę linię
...
bool init() {
    ...
    uniformScaleLoc = glGetUniformLocation(program, "scale"); // <-- Dodano tę linię
    return true;
}
...
void render() {
    ...
    glUseProgram(program);
    glUniform1f(uniformScaleLoc, 0.3f); // <--- Ustawienie skali przed rysowaniem
