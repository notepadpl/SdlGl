...
void render() {
    ...
    // Obliczamy kierunek kamery na podstawie yaw i pitch
    glm::vec3 front;
    ...
    // Ustawiamy pozycję kamery na podstawie kierunku i odległości od centrum
    cameraPos = cameraDistance * (-cameraFront);

    // Teraz kamera patrzy z pozycji `cameraPos` na punkt (0,0,0)
        ...
        // --- OBSŁUGA DOTYKU ---
            ...
            if (numFingers == 1 && mouseDown) { // Ruch jednym palcem (obrót)
                ...
                if (finger1 && finger2 && initialFingerDistance > 0.001f) {
                    ...
                    cameraDistance = glm::clamp(cameraDistance, 1.0f, 10.0f);
                    initialFingerDistance = currentFingerDistance; // Aktualizujemy odległość
                }
            }
        }
    }
